...
#makotronics fixed our speed issue by converting
#the masking to a binary array, pretty cool
...
sThresh = settings.singleThreshold
...
gFontSingle = settings.gCodeSingle
        ...
        while fail == True:
            ...
            for i in range(0, h, 1):
                ...
                if ((backing_bits[i+Rboard_h] >> shift_by) & textBox_bits[i]) != 0:
                    ...
                    if (failcount % 2000) == 0:
                        consoleClass.thread3 = sub.userName + " placing. Fail" + str(failcount) + " times, avg: " + str(runtime/failcount)
...
def makeGcode(name):
    ...
    consoleClass.thread4 = name.userName
    
    #open the correct font file
    if name.fontSize >= sThresh:
        with open(gFontFile, "r") as searchFile:
            stringFile = searchFile.read()
    else:
        with open(gFontSingle, "r") as searchFile:
            stringFile = searchFile.read()
    ...
    consoleClass.thread4 = name.userName + " is " + str(entryString.count('\n')) + " lines"
